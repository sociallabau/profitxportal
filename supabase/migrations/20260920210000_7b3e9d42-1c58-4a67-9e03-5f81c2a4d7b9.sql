-- Supercharged Dan AI: a client-facing assistant, financial audits, and the
-- expense detail needed to actually diagnose a margin problem.

-- ---------------------------------------------------------------------------
-- 1. Dan's voice, analysed once and reused on every answer.
-- Derived from his own messages and teaching, so client-facing answers sound
-- like him without any private message ever being sent to a client.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. What clients ask. Dan sees the themes; each client sees only their own.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 3. Expense detail. Margin alone says something is wrong; these say what.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 4. Uploaded P&Ls and ad tracking sheets, with Dan's audit of them.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. The privacy boundary for the client-facing assistant.
--
-- The knowledge base holds 1:1 client call transcripts and WhatsApp threads.
-- Clients must never reach those. This function is the only retrieval path
-- the client-facing assistant uses, and it is restricted in SQL rather than in
-- application code so it cannot be bypassed by a mistake upstream.
--   teaching   = momentum calls, Q&As, workshops, hot seats
--   transcript = Dan's own trainings and uploaded material
-- Excluded: call (1:1s), note (Gemini notes of 1:1s), whatsapp (private threads)
-- ---------------------------------------------------------------------------
