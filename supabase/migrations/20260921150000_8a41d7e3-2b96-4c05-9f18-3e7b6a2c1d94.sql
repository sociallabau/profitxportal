-- Monthly reviews: a short round-up for the client, a full breakdown for Dan.
--
-- The two live in one row but must not be equally readable. The table is
-- admin-only, and clients reach their own summary through a function that
-- returns that column and nothing else — so the split is enforced in the
-- database rather than by remembering not to render the wrong field.
...
-- A client's own round-up, and only the round-up.
